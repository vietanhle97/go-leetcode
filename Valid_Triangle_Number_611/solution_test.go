...
func Test_validTriangleNum(t *testing.T) {
	type args struct {
		nums []int
	}
	tests := []struct {
		...
		},
		{
			name: "valid triangle number",
			args: args{
				nums: []int{2, 2, 3, 4, 5, 6, 7, 8, 9, 1, 2, 3, 4, 5, 7, 3, 3, 6, 3, 2, 10, 4, 5, 2, 4},
			},
			want: 1097,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.want, triangleNumber(tt.args.nums))
		})
	}
}
